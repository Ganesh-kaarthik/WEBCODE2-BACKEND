...
module.exports = {
    ...
    async deleteQuestion(req, res) {
        ...
        try {
            ...
            await User.findOneAndUpdate({ _id: req.user._id }, {
                $pull: { userQuestions: req.params.id, userAnswers: { $in: question.answers } }
            });
    ...
    async searchQuestionByTag(req, res) {
        ...
        try {
            ...
            const questions = await Question.find({ tags: { $in: req.params.tag } })
    ...
    async getTags(req, res) {
        ...
        try {
            ...
            const tagsArray = tagsArrayOfArrays.reduce((flatten, arr) => [...flatten, ...arr], []);
    ...
    async searchQuestion(req, res) {
        ...
        try {
            ...
            const questions = await Question.find({
                $and: [{
                    $or: [{ body: keywordRegex.body }, { title: keywordRegex.title },
                    { tags: { $in: keyword } }]
                }]
            }).populate("userId", "userName");
    ...
    async voteQuestion(req, res) {
        ...
        try {
            ...
            if (req.body.vote === "up") {
                ...
                question.downVotes = question.downVotes.filter(v => v.toString() !== req.user._id.toString());
                ...
                question.upVotes = question.upVotes.filter(v => v.toString() !== req.user._id.toString());
